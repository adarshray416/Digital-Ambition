...
if ( !class_exists('Themify_Updater') ) :
    ...
    class Themify_Updater
    {
        ...
        public function __construct()
        {
	        ...
	        $themify_aa = new Themify_Auto_Update($this->settings , $this->version, $this->license);
        ...
        public function menu_p() {
            ...
            if ( isset($_GET['page'],$_POST['updater_licence']) && $_GET['page'] === 'themify-license') {
                ...
                $autoUpdate = isset($_POST['autoUpdate']) ? true : false;
                ...
                $settings = array('key' => $credential['updater_licence'], 'username' => $credential['themify_username'], 'hideKey' => $hideKey, 'hideName' => $hideName, 'autoUpdate' => $autoUpdate, 'hideNotice' => $hideNotice, 'notification' => $notification, 'noticeEmail' => $noticeEmail);
            ...
            if( !empty( $installed_plugins ) ) {
                foreach ( $installed_plugins as $key => $plugin ) {
                    ...
                    if ( ( !$this->version->has_attribute($plugin_name, 'wp_hosted') && $this->version->is_update_available($plugin_name, $plugin['Version']) )
                         || ( Themify_Updater_utils::is_admin_penal() && isset($_GET['themify_theme_downgrade'], $_GET['plugin']) && urldecode( $_GET['plugin'] ) === $plugin_name )
                    ) {
            ...
            if( !empty( $installed_themes ) ) {
                foreach ($installed_themes as $key => $theme) {
                    if ( ( !$this->version->has_attribute($key, 'wp_hosted') && $this->version->is_update_available($key, $theme->get('Version')) )
                            || ( Themify_Updater_utils::is_admin_penal() && isset($_GET['themify_theme_downgrade'], $_GET['theme']) && urldecode( $_GET['theme'] ) === $key )
						)
                    {
                        $temp_class = new stdClass;
                        $temp_class->name = $key;
            ...
            if (!is_array($transients)) {
                ...
                foreach ($updates as $update) {
                    ...
                    if ( !empty($products) && in_array($update->slug, $products ) ) {
                        if ( Themify_Updater_utils::is_admin_penal() &&  isset($_GET['themify_theme_downgrade']) ) {
        ...
        public function upgrader_error_message($reply, $package , $upgrader) {
            ...
            if (  $this->version->is_update_available($name, $version) ) {
                ...
                } elseif ( ! $this->license->has_product_access($name) ) {
                    $upgrader->strings['no_package'] = $upgrader->strings['skin_update_failed'] = $upgrader->strings['skin_update_failed_error'] = sprintf( __('Sorry you don\'t have access to %s updates', 'themify-updater'), $nicename );
                }
            }
            return $reply;
        }
